...
export const generatePDF = async (formData = {}, returnBlob = false) => {
  ...
  console.log("ProcessedData reviewSignData:", processedData.reviewSignData);
  console.log("SafeFormData reviewSignData:", safeFormData.reviewSignData);
  ...
  try {
    ...
    const drawLabeledLine = (
      ...
      if (dynamicWidth && safeValue && safeValue !== "Not provided") {
        ...
        // Add smaller padding for more precise sizing
      ...
      if (shouldWrapText && safeValue && safeValue !== "Not provided") {
        ...
        lines.forEach((line, index) => {
          drawUnderline(page, lineStartX, lineEndX, y - 2 - index * lineHeight);
        });
      } else {
    ...
    if (lineCount > 0) {
      for (let i = 0; i < lineCount; i++) {
        const lineY = notesTopY - 10 - (i * 15);
        page.drawLine({
          start: { x: 52, y: lineY },
          end: { x: width - 42, y: lineY },
          thickness: 0.5,
          color: rgb(0, 0, 0),
        });
      }
    ...
    const drawClientInfoVertically = (client1, client2, startY) => {
      ...
      drawLabeledLine(page, {
        ...
        x: rightX,
        y: currentY,
        lineWidth: 80, // Reduced to match content
        labelWidth: 35,
        dynamicWidth: true,
        minWidth: 60,
        maxWidth: 100,
      ...
      drawLabeledLine(page, {
        label: "First Names",
        value: client1.firstName || "",
        x: leftX,
        y: currentY,
        lineWidth: 150, // Reduced to match content
        labelWidth: 60,
        dynamicWidth: true,
        minWidth: 100,
        maxWidth: 200,
      ...
      drawLabeledLine(page, {
        ...
        x: rightX,
        y: currentY,
        lineWidth: 100, // Reduced to match content
        labelWidth: 70,
        dynamicWidth: true,
        minWidth: 80,
        maxWidth: 120,
      ...
      drawLabeledLine(page, {
        label: "Last Name",
        value: client1.lastName || "",
        x: leftX,
        y: currentY,
        lineWidth: 250, // Reduced to match content
        labelWidth: 60,
        dynamicWidth: true,
        minWidth: 150,
        maxWidth: 300,
      ...
      drawLabeledLine(page, {
        label: "Address",
        value: client1.address || "",
        x: leftX,
        y: currentY,
        lineWidth: 300, // Reduced to match content
        labelWidth: 50,
        shouldWrapText: true,
        lineHeight: 12,
        dynamicWidth: true,
      ...
      drawLabeledLine(page, {
        label: "Marital Status",
        value: client1.maritalStatus || "",
        x: leftX,
        y: currentY,
        lineWidth: 120, // Reduced to match content
        labelWidth: 70,
        dynamicWidth: true,
        minWidth: 100,
        maxWidth: 150,
    ...
    drawYesNoRow(
      ...
      {
        label: "",
        value: previousWill ? "yes" : "no",
        x: 150, // Position next to the heading
        y: currentY, // Same Y coordinate as heading
        fontSize: 10,
        gap: 20,
      },
    );
    ...
    drawYesNoRow(
      page,
      {
        label: "", // Label is already drawn
        value: meetingAttendees.isAnyoneElsePresent ? "yes" : "no",
        x: 300, // Position next to the question
        y: currentY, // Same Y coordinate as question
        fontSize: 10,
        gap: 20,
      },
    );
    ...
    y2 -= 20
    // Disinheritance heading
    page2.drawText("Disinheritance", {
      x: 35,
      y: y2 + 15,
      size: 14,
      font: boldFont,
    ...
    if (
      ...
      wishes.burial ||
      wishes.undecided ||
      funeral.client1Other
    ) {
      page4.drawText("Client 1 Funeral Wishes", {
        x: 50,
        y: y4,
        size: 12,
        font: boldFont,
    ...
    if (Array.isArray(client1Executors) && client1Executors.length > 0) {
      client1Height += client1Executors.length * 32;
      client1Height += (client1Executors.length - 1) * 4;
      client1Executors.forEach((ex) => {
        const address = getSafeValue(ex, "address", "");
        if (address) {
          const lines = wrapText(address, font, width - 220, 9);
          if (lines.length > 1) {
            client1Height += (lines.length - 1) * 12;
          }
        }
      });
    }
    ...
    if (Array.isArray(client2Executors) && client2Executors.length > 0) {
      client2Height += client2Executors.length * 32;
      client2Height += (client2Executors.length - 1) * 4;
      client2Executors.forEach((ex) => {
        const address = getSafeValue(ex, "address", "");
        if (address) {
          const lines = wrapText(address, font, width - 220, 9);
          if (lines.length > 1) {
            client2Height += (lines.length - 1) * 12;
          }
        }
      });
    }
    ...
    const drawExecutorList = ({ label, list, topY, isClient1 = false }) => {
      ...
      if (Array.isArray(list) && list.length > 0) {
        ...
        list.forEach((ex) => {
          ...
          if (address) {
            const lines = wrapText(address, font, width - 220, 9);
            if (lines.length > 1) {
              sectionHeight += (lines.length - 1) * 14; // Increased extra height for additional lines (was 12)
            }
          }
        });
      } else {
      ...
      drawRoundedRect(
        page4,
        40, // Moved left to accommodate more content
        ey - sectionHeight + 8,
        width - 60, // Increased width to prevent address overflow
        sectionHeight,
        8, // Rounded corners radius
      ...
      list.forEach((ex, idx) => {
        ...
        drawLabeledLine(page4, {
          label: "Executor Type",
          value: getSafeValue(ex, "executorType", ""),
          x: 50,
          y: ey,
          lineWidth: 200,
          labelWidth: 70,
    ...
    if (Array.isArray(bequests) && bequests.length > 0) {
      bequests.forEach((bequest, idx) => {
        ...
        drawRoundedRect(
          page4,
          35,
          y4 - bequestHeight + 25, // Increased to include heading inside background
          width - 70,
          bequestHeight,
          8, // Same radius as case notes
          isEven ? rgb(0.94, 0.94, 0.95) : rgb(0.98, 0.98, 0.98), // Darker for odd, lighter for even
        ...
        if (classGiftText) {
          ...
          y4 -= classGiftHeight + 5; // Add extra spacing after wrapped text
        } else {
        ...
        if (bequest.giftType === "Money") {
          ...
          drawLabeledLine(page4, {
            ...
            x: 50,
            y: y4,
            lineWidth: 350, // Reduced from 400 to add right-side spacing
            labelWidth: 100,
            shouldWrapText: true,
            lineHeight: 12,
            dynamicWidth: false, // Fixed width to prevent overflow
            minWidth: 200,
            maxWidth: 350, // Reduced from 450 to match lineWidth
          });
          y4 -= 30;
        }
        ...
        if (otherConditionsText) {
          ...
          drawLabeledLine(page4, {
            ...
            x: 50,
            y: y4,
            lineWidth: 380, // Reduced width to fit within grey box
            labelWidth: 100,
            dynamicWidth: false,
            shouldWrapText: true,
            lineHeight: 12,
    ...
    if (Array.isArray(propertyTrusts) && propertyTrusts.length > 0) {
      propertyTrusts.forEach((trust, idx) => {
        ...
        if (idx === 0) {
          // First trust - use current position with very low threshold
          if (y4 < 150) { // Much lower threshold for first trust
            page4 = pdfDoc.addPage([width, height]);
            y4 = height - 60;
          }
        } else {
          // Subsequent trusts - always use new page
          if (y4 < 350) {
            page4 = pdfDoc.addPage([width, height]);
            y4 = height - 60;
          }
        ...
        drawLabeledLine(page4, {
          ...
          x: 50,
          y: y4,
          lineWidth: 150, // Reduced from 200 to add right space
          labelWidth: 80,
          dynamicWidth: true,
          minWidth: 100,
        ...
        drawLabeledLine(page4, {
          ...
          x: 50,
          y: y4,
          lineWidth: 350, // Reduced from 400 to add right space
          labelWidth: 100,
          shouldWrapText: true,
          lineHeight: 12,
          dynamicWidth: true,
        ...
        if (
          ...
          if (occupantText.length > 0) {
            drawLabeledLine(page4, {
              ...
              x: 50,
              y: y4,
              lineWidth: 320, // Adjusted for optimal right space
              labelWidth: 120,
              shouldWrapText: true,
              lineHeight: 12,
              dynamicWidth: true,
              minWidth: 200,
              maxWidth: 350, // Reduced for right space
            });
            y4 -= 20; // Increased from 18 for more spacing
          }
        }
        ...
        if (trust.downsizingOther) {
          ...
          page4.drawText("In event of downsizing, what happens to surplus?", {
            x: 50,
            y: y4,
            size: 10,
            font: boldFont,
        ...
        if (trust.trustPeriodEndsOther) {
          ...
          page4.drawText("Who are the beneficiaries when trust period ends", {
            x: 50,
            y: y4,
            size: 10,
            font: boldFont,
    ...
    if (classGiftValue) {
      ...
      y4 -= classGiftHeight + 5; // Adjust y4 by actual height
    } else {
      y4 -= 20; // Default spacing if no content
    }
      ...
      alternateGroups.forEach((group, idx) => {
        ...
        if (group.beneficiaries && Array.isArray(group.beneficiaries) && group.beneficiaries.length > 0) {
          ...
          drawRoundedRect(
            page4,
            40,
            y4 - altBeneficiariesHeight + 5, // Reduced padding
            width - 60,
            altBeneficiariesHeight,
            8,
            rgb(0.94, 0.94, 0.94),
            rgb(0.8, 0.8, 0.8)
          );
          ...
          if (group.goc && group.goc.otherEntities !== undefined) {
            drawLabeledLine(page4, {
              ...
              value: group.goc.otherEntities ? "Yes" : "No",
              x: 50,
              y: altBeneficiaryY,
              lineWidth: 150,
              labelWidth: 80,
          ...
          if (group.goc && group.goc.giftOverToChildren !== undefined) {
            drawLabeledLine(page4, {
              label: "Gift Over to Children:",
              value: group.goc.giftOverToChildren ? "Yes" : "No",
              x: 50,
              y: altBeneficiaryY,
              lineWidth: 150,
              labelWidth: 120,
          ...
          if (group.goc && group.goc.accruer !== undefined) {
            drawLabeledLine(page4, {
              label: "Accruer:",
              value: group.goc.accruer ? "Yes" : "No",
              x: 50,
              y: altBeneficiaryY,
              lineWidth: 150,
              labelWidth: 80,
        ...
        if (group.hasSubstitutionalProvisions !== undefined) {
          drawLabeledLine(page4, {
            label: "Substitutional Provisions:",
            value: group.hasSubstitutionalProvisions ? "Yes" : "No",
            x: 50,
            y: y4,
            lineWidth: 150,
            labelWidth: 140,
    ...
    if (fpBeneficiaryGroups.length > 0) {
      ...
      fpBeneficiaryGroups.forEach((group, gIndex) => {
        ...
        if (goc.giftOverToChildren !== undefined || goc.accruer !== undefined || goc.otherEntities !== undefined || goc.furtherDetails) {
          // GOC Heading
          pageLPA.drawText("Gift Over Clause (GOC) - if gift fails", {
            x: 70,
            y: yLPA,
            size: 10,
            font: boldFont,
    ...
    if (!reviewSignData || Object.keys(reviewSignData).length === 0) {
      try {
        const localStorageData = localStorage.getItem('planning_bee_app_data');
        if (localStorageData) {
          const parsedData = JSON.parse(localStorageData);
          finalReviewSignData = parsedData.reviewSignData || {};
          console.log("=== FALLBACK: Using localStorage reviewSignData ===");
          console.log("LocalStorage reviewSignData:", finalReviewSignData);
        }
      } catch (error) {
    ...
    if (finalReviewSignData.paymentTerms && finalReviewSignData.paymentTerms.trim() !== "") {
      ...
      paymentTermsLines.forEach((line, index) => {
        pageLPA.drawText(line, {
          x: 52,
          y: paymentTermsBoxY + paymentTermsBoxHeight - 16 - (index * 12),
          size: 9,
          font,
          color: rgb(0, 0, 0),
        });
    ...
    if (safeFormData.idInformation) {
      ...
      console.log("ReviewSignData in safeFormData:", safeFormData.reviewSignData); // Check reviewSignData
      console.log(
...
const generateReceiptPDF = async (formData = {}) => {
  try {
    ...
    if (returnBlob) {
      ...
      saveAs(blob, `planning-bee-receipt-${timestamp}.pdf`);
      return true;
    }
  } catch (error) {
