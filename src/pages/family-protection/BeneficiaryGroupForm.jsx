...
const BeneficiaryGroupForm = ({
  ...
  const handlePersonSelect = (beneficiaryId, selectedValue) => {
    console.log("=== BENEFICIARY PERSON SELECT DEBUG ===");
    ...
    if (selectedPerson) {
      ...
      if (currentBeneficiary) {
        // Update beneficiary object with selected person's details
        const updatedBeneficiary = {
          ...currentBeneficiary,
          title: selectedPerson.title || currentBeneficiary.title,
        ...
        console.log("Calling onBeneficiaryChange with:", group.id, beneficiaryId, updatedBeneficiary);
        
        // Update the beneficiary with the new values
        onBeneficiaryChange(group.id, beneficiaryId, "update", updatedBeneficiary);
      } else {
        console.log("No current beneficiary found with ID:", beneficiaryId);
      }
    } else {
        ...
        <div className="space-y-2">
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              checked={group.goc?.giftOverToChildren || false}
              onChange={() => onGroupChange(group.id, "goc", {
                ...group.goc,
                giftOverToChildren: !group.goc?.giftOverToChildren
              })}
            />
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              className="w-3 h-3 md:w-4 md:h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              checked={group.goc?.accruer || false}
              onChange={() => onGroupChange(group.id, "goc", {
                ...group.goc,
                accruer: !group.goc?.accruer
              })}
            />
          ...
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
              className="w-3 h-3 md:w-4 md:h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              checked={group.goc?.otherEntities || false}
              onChange={() => onGroupChange(group.id, "goc", {
                ...group.goc,
                otherEntities: !group.goc?.otherEntities
              })}
            />
            <span className="text-xs md:text-sm text-gray-700">
